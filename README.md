...
Token efficiency (the Learn MCP server itself is free — cost is model tokens): requirements making the **identical capability claim** (same feature relied on + same proposed approach) are verified once per run and share the resulting citation (`API calls saved` in the run output). **Only confirmed verdicts are shared** — an unconfirmed result may be an artefact of one search, so every row retries the claim for itself rather than inheriting a miss. The verify prompt nudges toward cheap excerpts over whole-page fetches, but sets no call cap: a claim abandoned early becomes manual work for a consultant, which costs more than the saved tokens. No cost measure can produce a false VERIFIED — they can only ever downgrade a row to UNCONFIRMED, because the HTTP liveness guard is unchanged. Prompt caching is enabled on the fixed system+tools prefix of every stage (`llm.cache_prompt`, `verify.cache_prompt`), though fitgap's prefixes are short enough that it only engages on models with a low minimum cacheable size — the run summary states plainly when it didn't, rather than leaving it silent. Further knobs live under `verify:` in `fitgap.yaml` (`search_only`, a cheaper `model` for verification only, `max_searches`); see [docs/configuration.md](docs/configuration.md#prompt-caching). Measure any change with `fitgap benchmark-verify` — it reports cost per claim *and* how many claims still confirm — and re-run `fitgap eval` to confirm the accuracy gates still pass.
